...
Let us note the loss (error) as $`L`$ and the learning rate as $`\eta`$, and using the [*Chain rule*](https://en.wikipedia.org/wiki/Chain_rule), we will calculate the derivative of the loss w.r.t the parameter and update the patameter. 
