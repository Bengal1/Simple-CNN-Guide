...
The Simple CNN also use methods to accelerate convergence of the network training and avoid overfitting. After the second layer (Max-pool) the Simple CNN applies [*Dropout*](https://en.wikipedia.org/wiki/Dilution_(neural_networks)), and after the fourth layer (also Max-pool) it applies [*Batch-Normalization*](https://en.wikipedia.org/wiki/Batch_normalization).
